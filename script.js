window.onload = function() {
    ...
    if (mapBtn) {
        mapBtn.onclick = function() { mapBox.style.display = "block" }
    }
    if (closeMap) {
        closeMap.onclick = function() { mapBox.style.display = "none" }
    }
    ...
    if (chatBtn) {
        chatBtn.onclick = function() { chatBox.style.display = "flex" }
    }
    if (closeChat) {
        closeChat.onclick = function() { chatBox.style.display = "none" }
    }
    ...
    if (sendBtn && messages) {
        sendBtn.onclick = function() {
            ...
            setTimeout(function() { addMessage(reply(text), "bot") }, 500)
            input.value = ""
        }
    }
 
    const voiceBtn = document.getElementById("voiceBtn")
    if (voiceBtn && messages) {
